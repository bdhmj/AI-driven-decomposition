...
**По умолчанию срок и стоимость считаются только по задачам с `phase: "mvp"`.** Post-MVP задачи видны в листе «Оценка» с пометкой `[Post-MVP]`, но не входят в итоговые цифры. Это правило действует и при текстовом расчёте стоимости без xlsx — всегда фильтруем MVP. Чтобы включить post-MVP в расчёт, передавай флаг `--include-post-mvp`.
